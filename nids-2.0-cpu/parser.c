...
void buildfailfunc(RuleSetRoot *rsr)
{
	...
	while(tail_queue != flag_queue)
	{
		...
		{
			if(rsr->acArray[state][i] != 0)
			{// printf("Continue processing the queue\n"); // For Debug
				tail_queue++;
				tmp_queue[tail_queue] = rsr->acArray[state][i]; //& 0x0ffff;
				int tmp_state = 0;
...
void transContPattMatch(RuleSetRoot *rsr, AcNode *acnode)
{
...
void transmission(RuleSetRoot *rsr)
{
	// fit acArray in GPU
	transAc(rsr);
